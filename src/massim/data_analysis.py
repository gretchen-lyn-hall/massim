...
def build_sample_graph(masses: pd.Series|np.ndarray,
    ...
    if USE_MP:
        xfrms = find_xfrms_py(masses, keys[key_column], err_abs)
    else:
        xfrms = np.array(find_transforms(masses, keys[key_column], err_abs, False))
    ...
    G = make_graph(masses.shape[0], edges, vdata=vertex_data, edata=edge_data, use_mp=USE_MP)
...
def baseline_distances_2d(prepped, khist):
    ...
    # histogram, it will be the maximum dimension.
    max_col = max(khist.attrs["bins_x"], khist.attrs["bins_y"])
...
def unit_histogram_like(hist1, hist2):
    if hist1.shape[1] < hist2.shape[1]:
        new_cols = set(hist2.columns).difference(hist1.columns)
        hist1[sorted(list(new_cols))] = 0
    else:
        new_cols = set(hist1.columns).difference(hist2.columns)
        hist2[sorted(list(new_cols))] = 0
    hist1.attrs["bin_width"] = 1
    hist1.attrs["bin_min"] = min(hist1.columns)
    hist1.attrs["bin_num"] = len(hist1.columns)
    hist2.attrs.update(hist1.attrs)
    return hist1, hist2
    
            


def test_stats(test_da):
...
def build_baseline(test_stats, bin_method="doane", summary_method="all"):
    ...
    for mat_name in ALL_2D_MATS:
        ...
        # The maximum possible wasserstein distance will be the maximum
        # number of bins in either direction - 1
        dists /= (max(mat.attrs["bins_x"], mat.attrs["bins_y"]) - 1)
...
def robust_z_score(bl, test_comp, key):
    test_dists = test_comp[key]
    bl_kde = bl['kdes'][key]
    bl_dat = bl_kde.dataset[0]
    median = np.median(bl_dat)
    MAD = ST.median_abs_deviation(bl_dat)
    return (test_dists - median) / (1.4826 * MAD)

def quantile_score(bl, test_comp, key):
    """
...
def compare_spectra(test_stats, baseline, metrics=None):
    ...
    for mat in submetrics:
        robust_zs[mat] = root_mean_square(robust_z_score(baseline, result, mat), axis=1)
        
        quantiles[mat] = root_mean_square(
